	...
	Main.LoadPluginFile = function(pluginDir)
		if env.readfile then
			if isfile(pluginDir) then
				local ok1, preloadedPlugin = pcall(function()
					return (loadfile and loadfile(pluginDir)) or loadstring(env.readfile(pluginDir))
				end)
				if not ok1 or not preloadedPlugin then return nil end

				local ok2, loadedPlugin = pcall(preloadedPlugin)
				if not ok2 or not loadedPlugin then return nil end

				local control = loadedPlugin
				pcall(control.InitDeps, Main.GetInitDeps())

				local ok4, moduleData = pcall(control.Main)
				if not ok4 or not moduleData then return nil end
		...
		if env.readfile and listfiles then
			if #listfiles("dex/plugins") > 0 then
				...
				for _, pluginDir in pairs(listfiles("dex/plugins")) do
					local ok, moduleData = pcall(Main.LoadPluginFile, pluginDir)
					if not ok or not moduleData then continue end

					moduleData.PluginData = moduleData.PluginData or {}

					local initOk, initErr = pcall(moduleData.Init, moduleData)
					if not initOk then warn("Plugin Init error: "..tostring(initErr)) continue end

					local pluginFriendlyName = moduleData.PluginData.FriendlyName
						or (moduleData.Window and moduleData.Window.GuiElems and moduleData.Window.GuiElems.Title and moduleData.Window.GuiElems.Title.Text)
						or "Unnamed Plugin"
					local pluginName = moduleData.PluginData.Name
						or (moduleData.Window and moduleData.Window.GuiElems and moduleData.Window.GuiElems.Title and moduleData.Window.GuiElems.Title.Text:gsub(" ",""))
						or "unnamedPlugin"
...
local function main()
	...
	ds.Init = function()
		local window = lib_ref.Window.new()
		...
		-- ImageButton toggle igual ao ImageButton_4 original
		...
		-- TextLabel "PlayerData" igual ao TextLabel_5 original
		...
		-- TextButton de player igual aos TextButton_6/7/8/9 originais
		local function criarBotao(nome, yPos)
		...
		local function rebuild()
			...
			selectedButton = nil
			local idx = 0
			local bLocal = criarBotao((LocalPlayer and LocalPlayer.Name) or "Unknown", 24 + idx * 18)
			bLocal.Visible = isListVisible
			table.insert(playerButtons, bLocal)
			idx = idx + 1
			for _, plr in ipairs(Players:GetPlayers()) do
				if plr ~= LocalPlayer then
					local b = criarBotao(plr.Name, 24 + idx * 18)
					b.Visible = isListVisible
					table.insert(playerButtons, b)
					idx = idx + 1
				end
			end
			scrollFrame.CanvasSize = UDim2.new(0, 0, 0, math.max(24 + idx * 18 + 10, 30))
			if isListVisible then
				window:Resize(246, math.min(24 + idx * 18 + 10, 400))
			else
				window:Resize(246, 24)
			end
		end
		...
		imageButton.MouseButton1Click:Connect(function()
			...
			for _, btn in ipairs(playerButtons) do btn.Visible = isListVisible end
			if isListVisible then
		...
		Players.PlayerRemoving:Connect(function()
			coroutine.wrap(function() wait() rebuild() end)()
		end)
